	...
	ccg.prototype.getMediaFiles = function(cb) {
		...
		self.sendCommand("CLS", function(err, data) {
			...
			for (var index in files) {
				...
				var fileName = file.substring(file.indexOf('"') + 1, file.search('"  '));
				file = file.substring(file.search('"  ') + 3, file.length);
				file = file.split(' ');
				...
				out.push({
					...
					frames: parseInt(file[4]),
					frameRate: parseFloat(file[5].split('/')[1]) / parseFloat(file[5].split('/')[0])
				});
			}
